...
void capturador_pantalla::capturar() {
    ...
    std::wstring wstr_comando = this->configuracion.carpeta_utiles.wstring() + L"\\ffmpeg.exe -f gdigrab "
        L"-framerate " + std::to_wstring(configuracion.grabacion.fps) + L" -i desktop -vf scale=" + ancho + L":" + alto + L" -f dshow -i audio=\"Micrófono (Realtek High Definition Audio)\" " + wpath;
